...
export function savePhDataToDb(data) {
  ...
  request.onsuccess = (event) => {
    ...
    if (cursor) {
      ...
      const lastTimestamp = lastSavedData.phDataReceivedAt;
      ...
      if (newTimestamp === lastTimestamp) { // Atau new Date(newTimestamp).getTime() === new Date(lastTimestamp).getTime()
...
export function saveHumidityDataToDb(data) {
    ...
    if (cursor) {
      ...
      // Cek apakah humidityValue yang akan disimpan sama dengan humidityValue record terakhir
      if (lastSavedHumidityValue === data.humidityValue) {
